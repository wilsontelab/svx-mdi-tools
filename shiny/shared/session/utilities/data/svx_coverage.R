...
svx_rebinCoverage <- function(x0, aggFactor = 10){
...
svx_setCoverageValue <- function(bins, plotAs, medianCoverage, medianPloidy){
    bins[, .(
        strand = ".",
        x = x,
        y = switch(
            plotAs,
            "Read Depth" = y,
            y / medianCoverage * medianPloidy # Copy Number (possibly converted later to Copy Number Change)
        )
    )]
}
