...
parseEdgeMetadata <- function(edges, chromWindows){
    message("expanding edge metadata")
    edges <- cbind(
        edges, 
        edges[, parseSignedWindow(chromWindows, node1, 1)], 
        edges[, parseSignedWindow(chromWindows, node2, 2)]
    )
    setkey(edges, qName)
...
checkIndelBandwidth <- function(edges){
    message("checking block bandwiths to identify low quality base strethces with false indels")
    nodePairs <- list()
