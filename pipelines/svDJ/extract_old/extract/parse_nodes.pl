...
# as compared to other svX extractions
#   svDJ focuses specifically on internal split junctions
#   assumes there is only one amplicon per input sample (i.e., previously demultiplexed if needed)
...
# working variables
use vars qw(@nodes    @types    @mapQs    @sizes    @insSizes    @outAlns
...
sub parseReadAlignments {
    ...
    foreach my $i(0..$#alnIs){
        ...
        (@alnNodes, @alnTypes, @alnMapQs, @alnSizes, @alnInsSizes, @alnAlns) = ();
        parseSvsInCigar($aln, RSTART, REND, $$aln[CIGAR], \&commitAlignmentNodes);

        # maintain proper 5'-3' node order on bottom strand, since svDJ tracks genome paths
        if($$aln[STRAND] eq "-"){
...
sub commitAlignmentNodes{
    ...
    push @alnNodes, ( 
        join("ZZ", @$aln[RNAME, STRAND], $$aln[RSTART] + 1,  @$aln[SEQ, QUAL], $cigar), # svDJ nodes are always 1-referenced
        join("ZZ", @$aln[RNAME, STRAND, REND, SEQ, QUAL], $cigar)
    );
...
sub getSvSize { # always a positive integer, zero if NA
    ...
    $jxnType eq DUPLICATION and return $$innData1[_POS] - $$innData2[_POS] + 1;
    return 0;
}
sub getJxnStructure {
    my ($aln1, $aln2, $innData1, $innData2) = @_;
