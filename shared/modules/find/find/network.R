...
getMatchingNodes <- function(nodeName, nodeN, nodeClass){
    ...
    nodes    <- getNodes('nodes_by_proximity', nodeName, unpackNodeNames = TRUE)
...
followSvJunction <- function(edge){
    ...
    matchingNodes1 <- getMatchingNodes(edge$NODE1, 1L, edge$NODE_CLASS)
    matchingNodes2 <- getMatchingNodes(edge$NODE2, 2L, edge$NODE_CLASS)
    ...
    junctionNodes[, IS_SEED_NODE := NODE == ..junctionName]
