...
new_nbinomCountsGC <- function(
    ...
    label = NULL # if not null, report error message with this label, e.g., a cell ID
){ 
...
nb <- tryCatch(
    ...
    error = function(e) {
        if(!is.null(label)) message(paste("MASS::glm.nb failed", label, sep = " - "))
