...
gcIndices  <- minGcIndex:maxGcIndex # all GC steps from min to max value
...
fits <- sapply(gcIndices, function(gci){
    ...
    nbFormula <- rpa ~ 1
    n <- length(rpa)
    if(n < 5) return(c(1, 1000, 0)) # unweight rare bins on edges of gc space
    tryCatch(
        { 
            x <- suppressWarnings(MASS::glm.nb(nbFormula, control = glm.control(maxit = maxit))) 
            c(exp(coef(x)), x$theta, n)
        },
        error = function(e) {
            x <- glm(nbFormula, family = poisson, control = glm.control(maxit = maxit))
            c(exp(coef(x)), 1000, n) # high theta, i.e, not over-dispersed
        }
    )
})
...
mu <- predict(mu)
theta <- fits[2, ]
theta <- lm(theta ~ gcIndices + gcIndices2, weights = fits[3, ])
theta <- predict(theta)
peak <- mapply(
    function(size, mu){
...
structure(
    list(
        ...
        mu = mu,       # poisson mean, as reads per allele
        peak = peak    # the most frequent reads per allele value in each GC bin
    ),
    class = class
)
